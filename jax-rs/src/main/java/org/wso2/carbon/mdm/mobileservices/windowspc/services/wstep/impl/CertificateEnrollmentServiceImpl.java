 ...
 * protocol.
 */
@WebService(endpointInterface = Constants.CertificateEnrollment.CERTIFICATE_ENROLLMENT_SERVICE_ENDPOINT,
		targetNamespace = Constants.CertificateEnrollment.DEVICE_ENROLLMENT_SERVICE_TARGET_NAMESPACE)
...
public class CertificateEnrollmentServiceImpl implements CertificateEnrollmentService {
	...
	public void setRootCertAndKey(String storePassword, String keyPassword)
		...
		File JKSFile = new File(getClass().getClassLoader().getResource(
				Constants.CertificateEnrollment.WSO2_MDM_JKS_FILE).getFile());
		...
		try {
			CAPrivateKey = (PrivateKey) securityJKS
					.getKey(Constants.CertificateEnrollment.CA_CERT, keyPassword.toCharArray());
		...
		try {
			certificateFactory =
					CertificateFactory.getInstance(Constants.CertificateEnrollment.X_509);
	...
	public String prepareWapProvisioningXML(String binarySecurityToken, List certPropertyList,
		...
		try {
			...
			CACertificatePosition.getParentNode().getAttributes().getNamedItem(Constants.
					              CertificateEnrollment.TYPE).setTextContent(String.
			...
			Node rootCertNode =
					rootCertAttributes.getNamedItem(Constants.CertificateEnrollment.VALUE);
			...
			Node clientEncodedNode =
					clientCertAttributes.getNamedItem(Constants.CertificateEnrollment.VALUE);
